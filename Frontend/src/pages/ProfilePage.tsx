...
<<<<<<< HEAD
  ...
  displayName: localStorage.getItem("currentUserName") || "Guest",
  bio: "",
  avatarUrl: ""
  });
...
const fetchMyProfile = async () => {
  try {
    ...
    // 1. Gọi hàm đúng từ userService
    const data = await userService.getCurrentProfileMe(); 
    
    // 2. Map dữ liệu khớp với UserProfileDto
    setUserProfile({
      ...data,
...
<<<<<<< HEAD
  ...
  const isSaveDisabled = isSavingProfile || isProfileLoading || !userProfile.id;
...
<<<<<<< HEAD
  ...
  try {
    ...
    // Backend check: userId lấy từ JWT claim phải == request.Id
    // Lấy userId trực tiếp từ token để tránh mismatch do state/localStorage.
    const token = localStorage.getItem('token');
    const tokenPayload = token ? JSON.parse(atob(token.split('.')[1])) : null;
    const jwtUserId = tokenPayload?.sub || tokenPayload?.nameid || tokenPayload?.['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier'];

    const updated = await userService.updateProfile({
      id: jwtUserId || userProfile.id,
      userName: userProfile.userName,
      displayName: editName, // Khớp với DisplayName trong DTO
      bio: editBio,
      avatarUrl
    });

    // Lưu ý: Backend ProfileController.UpdateProfile có check:
    // userId == request.Id (userId lấy từ JWT claim).
    // Nếu còn lỗi 400/403, phần sai gần như chắc chắn nằm ở request.Id/userId mismatch.

    // Cập nhật form trạng thái sau khi gọi API thành công
    // (giúp tránh tình trạng preview/avatarUrl bị lệch nếu BE trả lại khác)
    ...
    localStorage.setItem("currentUserName", updated.displayName || updated.userName);
    ...
    const message = backendMessage || error?.message || "Không thể lưu thay đổi. Vui lòng thử lại.";
    setProfileError(status ? `Request fail (${status}): ${message}` : message);

    // Giúp debug nhanh nguyên nhân (thường do payload/route không khớp)
    if (backendData) {
